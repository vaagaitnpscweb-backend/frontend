...
function TnpscQuiz() {
  ...
  // 🌍 1. அட்மின் அப்ரூவ் செய்த TNPSC PDFகளை டேட்டாபேஸில் இருந்து எடுத்தல்
  useEffect(() => {
    fetch(`${API_BASE}/api/paid-pdfs/client/TNPSC`)
      .then(res => res.json())
      .then(data => { 
  ...
  // 💳 2. Razorpay பேமெண்ட் ஃபங்ஷன் (தனிப்பட்ட PDF வாங்குவதற்கு)
  const handlePayment = async (pdfId, title, price) => {
    ...
    const confirmPay = window.confirm(`🏆 Vaagai Tuition - TNPSC Test\n\nProceed to pay ₹${price} via Razorpay to unlock this test?`);
  ...
  if (loading) return <div style={{ textAlign: 'center', padding: '50px', fontSize: '18px' }}>🔄 Loading TNPSC Tests...</div>;
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '30px auto', padding: '20px', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      ...
      <div style={{ background: '#002b49', color: 'white', padding: '20px', borderRadius: '8px', textAlign: 'center', marginBottom: '25px' }}>
        <h1 style={{ margin: '0 0 8px 0', fontSize: '24px' }}>🎯 TNPSC தேர்வுப் பெட்டகம்</h1>
        <p style={{ margin: 0, opacity: 0.9, fontSize: '14px' }}>உயர்தர மாதிரித் தேர்வுகள் மற்றும் பாடக் குறிப்புகள் (சப்ஸ்கிரிப்ஷன் வைத்துள்ள மாணவர்களுக்கு அனைத்து PDF-களும் முற்றிலும் இலவசம்!)</p>
      </div>

      {pdfList.length === 0 ? (
        <div style={{ padding: '40px', textAlign: 'center', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0', color: '#64748b' }}>
          👋 No TNPSC materials are currently live. Please check back later!
        </div>
      ) : (
