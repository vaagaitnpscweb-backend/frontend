...
function PcQuiz() {
  ...
  // 🌍 1. அட்மின் அப்ரூவ் செய்த PC PDFகளை டேட்டாபேஸில் இருந்து எடுத்தல்
  useEffect(() => {
  ...
  // 💳 2. Razorpay பேமெண்ட் ஃபங்ஷன் (Dynamic Price Handling)
  const handlePayment = async (pdfId, title, price) => {
    ...
    const itemPrice = Number(price) > 0 ? Number(price) : 5;
    const confirmPay = window.confirm(`🏆 Vaagai Tuition - PC Exam\n\nProceed to pay ₹${itemPrice} via Razorpay to unlock this test?`);
  ...
  if (loading) return <div style={{ textAlign: 'center', padding: '50px', fontSize: '18px' }}>🔄 Loading PC Exam Tests...</div>;
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '30px auto', padding: '20px', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      ...
      <div style={{ background: '#002b49', color: 'white', padding: '20px', borderRadius: '8px', textAlign: 'center', marginBottom: '25px' }}>
        <h1 style={{ margin: '0 0 8px 0', fontSize: '24px' }}>🛡️ PC (Police Constable) தேர்வுப் பெட்டகம்</h1>
        <p style={{ margin: 0, opacity: 0.9, fontSize: '14px' }}>காவலர் தேர்விற்கான பிரத்யேக மாதிரித் தேர்வுகள் மற்றும் பாடக் குறிப்புகள்</p>
      </div>

      {pdfList.length === 0 ? (
        <div style={{ padding: '40px', textAlign: 'center', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0', color: '#64748b' }}>
          👋 No PC Exam materials are currently live. Please check back later!
        </div>
      ) : (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', background: '#ffffff', textAlign: 'left' }}>
            ...
            <tbody>
              {pdfList.map((pdf, index) => {
                ...
                const currentPrice = pdf.price !== undefined && pdf.price !== null ? pdf.price : 0;
                ...
                return (
                  <tr key={pdf.id} style={{ borderBottom: '1px solid #e2e8f0', backgroundColor: index % 2 === 0 ? '#ffffff' : '#f8fafc' }}>
                    ...
                    {/* 3. Access Type / Dynamic Price Badge */}
                    ...
                    <td style={{ padding: '14px 20px', border: '1px solid #e2e8f0', textAlign: 'center' }}>
                      {!isPurchased ? (
                        <button 
                          onClick={() => handlePayment(pdf.id, pdf.title, currentPrice)} 
                          style={{ 
                            background: '#22c55e', 
                            color: '#ffffff', 
                            padding: '8px 16px', 
