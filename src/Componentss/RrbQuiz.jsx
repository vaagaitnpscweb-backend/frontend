...
function RrbQuiz() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/paid-pdfs/client/RRB`)
      .then(res => res.json())
      .then(data => { 
  ...
  const handlePayment = async (pdfId, title, price) => {
    ...
    const confirmPay = window.confirm(`🏆 Vaagai Tuition - RRB Test\n\nProceed to pay ₹${price} via Razorpay to unlock this test?`);
  ...
  if (loading) return <div style={{ textAlign: 'center', padding: '50px', fontSize: '18px' }}>🔄 Loading RRB Mock Tests...</div>;
    ...
    <div style={{ maxWidth: '1000px', margin: '30px auto', padding: '20px', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      ...
      {pdfList.length === 0 ? (
        <div style={{ padding: '40px', textAlign: 'center', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0', color: '#64748b' }}>
          👋 No RRB materials are currently live. Please check back later!
        </div>
      ) : (
